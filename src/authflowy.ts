...
export type AuthFlowyReturn<T extends readonly OAuthProvider<any>[]> = {
  '~providers': T;
  authorize: (
    id: Identifier<T>,
    data?: Record<string, unknown>
...
export type AuthFlowyConfig<T extends readonly OAuthProvider<any>[]> = {
  secret: string;
  providers: T;
};

export function defineAuthFlowyConfig<
  const T extends readonly OAuthProvider<any>[],
>(config: AuthFlowyConfig<T>): AuthFlowyReturn<T> {
  ...
  return {
    ...
    authorize: async (id, data = {}) => {
      ...
      if (!provider) {
        throw new AuthFlowyError(`Provider "${String(id)}" not found`);
      }
    ...
    callback: async (id, options) => {
      ...
      if (!decryptedState) {
        throw new AuthFlowyError('Invalid state');
      }
